...
public class GiveCapRoleCommand implements SlashCommand {
        ...
        public void handle(SlashCommandInteractionEvent event) throws Exception {
                ...
                if (member.getNickname() == null || (matchResult = regex.find(member.getNickname(), 0)) == null) {
                        event.reply("Ich weiß nicht, für welches Team du spielst. Bitte melde dich beim "
                ...
                if (!selfMember.canInteract(divRole) || !selfMember.canInteract(member)
                                || !selfMember.canInteract(newCaptain)) {
                        event.reply("Ich kann nicht mit den Usern oder mit den Rollen interagieren. Bitte melde dich beim "
                                        + ChannelReferences.HELPDESK + ".")
                                        .setEphemeral(true)
                                        .queue();
                        return;
                ...
                boolean hasCaptain = newCaptain.getRoles().stream().map(role -> role.getName().contains("Division"))
                ...
                Button accept = Button.success(
                                String.format("%d:CAPROLE:%d", event.getUser().getIdLong(), newCaptain.getIdLong()),
                                "Jo, stimmt alles!").asEnabled();
                Button decline = Button.danger(String.format("%d:CAPROLE:decline", event.getUser().getIdLong()),
                                "Nope, lieber nicht...").asEnabled();
        ...
        public CommandData commandData() {
                return Commands.slash("givecaprole",
                                "Übergibt deine Kapitänrolle an eine andere Person. Ist irreversibel, also sei vorsichtig.")
                                .addOption(OptionType.USER, "user",
                                                "Der User, welche deine Kapitänrolle bekommen soll.", true);
        }
}
